...
O modelo ajustado resulta no parametro da tabela \ref{tab:parcomp}, alguns resultado parecem condizentes com o da análise descritiva, outros são um pouco surpreendentes. Como observado a posição na empresa tem uma relação positiva com o salário e é significativa para o modelo. O efeito do sexo também foi significativo e o efeito para mulher é maior, ou seja é esperado que mulheres localizadas na média femina(posição e experiência) ganhem mais que homens localizados na média masculina. Por fim os anos de experiência parecem não afetar o salário para as mulheres, pois seu parâmetro foi não significativo e para os os parece afetar negativamente.
...
Observando os gráficos de resíduos na figura \ref{fig:diag1} não percedemos nenhuma evidência contra a indepêndencia dos dados ou contra a homocedásticidade. Porém através do histograma dos resíduos  e o gráfico quantil-quatil nota-se um pequena assimetria e também caudas mais pesadas, o que sugere que a suposição que os erros sigam uma normal não esteja correta.

```{r diag1, fig.cap="\\label{fig:diag1}Diagnóstico do modelo inicial"}
normal_diag(salary.fit)
```

Em relaação a pontos influentes e alavancas, podemos notar na figura \ref{fig:influ1} que varios pontos são candidatos. Sendo eles as observações: 4,191,163, 30 e 148. Porém ao retirá-los individualmente e ajusatarmos o modelo novamente as conclusões não mudam e as estimativas praticamente não se alteram.
...
O novo modelo ajustado resulta no parametro da tabela \ref{tab:parred}, as estimativas e as conlusões se mantém similares ao do modelo inicial e agora todos os parâmetros são significátivos.
...
Observando os gráficos de resíduos na figura \ref{fig:diag2} chegamos as mesmas conclusões que para o modelo inicial. Que as suposições de homcedásticidade e indepência são aceitaveis e a de normalidade não, que no caso do modelo reduzio pareceaté pior.

```{r diag2, fig.cap="\\label{fig:diag2}Diagnóstico do modelo reduzido"}
normal_diag(salary.fit.reduzido)
```

Também similar ao modelo incial temos alguns candidatos a pontos influentes ou alavanca, observados na figura \ref{fig:influ2}, mas que também ao serem retirados não alteram muito as estimativas ou a conclusão.
...
Através das medidas de comparação, vistas na tabela \ref{tab:medidas}, percebemos que para a maioria das medidas o modelo reduzido possue um valor mais baixo, então em teoria seria um modelo melhor.
